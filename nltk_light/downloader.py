...
class Downloader:
    ...
    def _pkg_status(self, info, filepath):
        ...
        if filepath.endswith(".zip"):
            ...
            unzipped_size = sum(
                os.stat(os.path.join(d, f)).st_size for d, _, files in os.walk(unzipdir) for f in files
            )
    ...
    def _update_index(self, url=None):
        ...
        if not (
            self._index is None or url is not None or time.time() - self._index_timestamp > self.INDEX_TIMEOUT
        ):
            return
        ...
        self._index = nltk_light.internals.ElementWrapper(ElementTree.parse(urlopen(self._url)).getroot())
    ...
    def default_download_dir(self):
        ...
        for nltkdir in nltk_light.data.path:
            if os.path.exists(nltkdir) and nltk_light.internals.is_writable(nltkdir):
                return nltkdir
...
class DownloaderGUI:
    ...
    def _init_widgets(self):
        ...
        for (i, (key, label, callback)) in enumerate(info):
            ...
            entry = Entry(infoframe, font="courier", relief="groove", disabledforeground="black")
    ...
    def _download(self, *e):
        ...
        marked = [
            self._table[row, "Identifier"] for row in range(len(self._table)) if self._table[row, 0] != ""
        ]
...
def _find_packages(root):
    ...
    """
    from nltk_light.corpus.reader.util import _path_from
